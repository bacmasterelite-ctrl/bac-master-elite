...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!geniusPublicKey || !geniusSecretKey)
    return res.status(500).json({ error: "Config GeniusPay manquante côté serveur." });

  const token = req.headers.authorization?.replace("Bearer ", "");
  if (!token) return res.status(401).json({ error: "Vous devez être connecté pour payer." });
  ...
  const webhookUrl = process.env["GENUISPAY_WEBHOOK_URL"] ?? "";
  ...
  try {
    ...
    if (!providerRes.ok)
      return res.status(providerRes.status).json({ error: "GeniusPay a refusé la transaction.", detail: providerJson || text });

    const checkoutUrl =
      (providerJson as any)?.data?.checkout_url ?? (providerJson as any)?.data?.payment_url;
    if (!checkoutUrl)
    ...
    return res.status(502).json({ error: "GeniusPay injoignable.", detail: err instanceof Error ? err.message : String(err) });
  }
}
