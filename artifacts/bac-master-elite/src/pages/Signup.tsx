...
const SERIES = [
  { code: "A", titre: "Série A", sub: "Littéraire", icon: Globe2, color: "from-fuchsia-500 to-violet-600" },
  { code: "C", titre: "Série C", sub: "Mathématiques", icon: Calculator, color: "from-blue-600 to-cyan-500" },
  { code: "D", titre: "Série D", sub: "Sciences", icon: Beaker, color: "from-emerald-500 to-teal-500" },
];
...
export default function Signup() {
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    if (!serie) {
      toast({ title: "Choisissez votre série", description: "La sélection de votre série est obligatoire.", variant: "destructive" });
      return;
    }
    setSubmitting(true);
    ...
    toast({ title: "Compte créé !", description: "Bienvenue sur BAC MASTER ELITE." });
      ...
      <div className="flex items-center justify-center p-6 sm:p-12 order-2 lg:order-1">
        ...
        >
          ...
          <form onSubmit={onSubmit} className="mt-8 space-y-5">
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="name" required value={fullName} onChange={(e) => setFullName(e.target.value)} placeholder="Aïcha Koné" className="pl-9" data-testid="input-fullname" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="email" type="email" required value={email} onChange={(e) => setEmail(e.target.value)} placeholder="vous@exemple.com" className="pl-9" data-testid="input-email" />
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input id="password" type={showPassword ? "text" : "password"} required minLength={6} value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Au moins 6 caractères" className="pl-9 pr-10" data-testid="input-password" />
            ...
            <div className="space-y-2">
              <Label>Votre série <span className="text-rose-600">*</span></Label>
              <div className="grid grid-cols-3 gap-2">
                {SERIES.map((s) => {
                  const active = serie === s.code;
                  return (
                    <button type="button" key={s.code} onClick={() => setSerie(s.code)}
                      className={cn("relative flex flex-col items-center gap-1.5 rounded-xl border-2 p-3 text-center transition-all hover-elevate", active ? "border-blue-600 bg-blue-50 dark:bg-blue-950/30" : "border-border bg-card")}
                    ...
                    >
                      ...
                      <div className={`flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br ${s.color} text-white shadow-sm`}>
          ...
          <p className="mt-6 text-center text-sm text-muted-foreground">
            Déjà un compte ?{" "}
            <Link href="/login" className="font-semibold text-blue-600 hover:underline">Se connecter</Link>
          </p>
        </motion.div>
      </div>
