...
import { ArrowLeft, CheckCircle2, Crown, Download, Loader2, Lock } from "lucide-react";
...
export default function Lecon() {
  ...
  const backUrl = subject
    ? `/dashboard/cours?subject=${encodeURIComponent(subject)}`
    : "/dashboard/cours";
  ...
  const handlePrint = () => {
    ...
    const addText = (text: string, size: number, bold: boolean, color: number[]) => {
      doc.setFont("helvetica", bold ? "bold" : "normal").setFontSize(size).setTextColor(color[0], color[1], color[2]);
    ...
    htmlDoc.body.querySelectorAll('h1,h2,h3,p,ul,table').forEach((node) => {
      const el = node as HTMLElement;
      const tag = el.tagName.toLowerCase();
      const text = el.textContent?.trim() || '';
      if (!text) return;
      if (y > pageH) { doc.addPage(); y = 60; }
      if (tag === 'h1' || tag === 'h2') { y += 8; addText(text, 14, true, [249, 115, 22]); }
      else if (tag === 'h3') { addText(text, 12, true, [249, 115, 22]); }
      else if (tag === 'p') { addText(text, 11, false, [30, 30, 30]); }
      else if (tag === 'ul') {
        ...
        if (rows.length > 0) {
          autoTable(doc, { head: [rows[0]], body: rows.slice(1), startY: y, theme: 'grid', styles: { fontSize: 10, cellPadding: 6 }, headStyles: { fillColor: [249, 115, 22], textColor: [255, 255, 255], fontStyle: 'bold' }, margin: { left: margin, right: margin } });
  ...
  if (isLoading) {
    return <DashboardLayout><div className="flex justify-center py-20"><Loader2 className="h-6 w-6 animate-spin" /></div></DashboardLayout>;
  }

  if (!lesson) {
    return <DashboardLayout><div className="text-center py-20">Leçon introuvable</div></DashboardLayout>;
  }
      ...
      <div className="mx-auto max-w-3xl space-y-6 pb-12">
            ...
            {!isPremium && (
              <Link href="/dashboard/upgrade">
                <Button variant="outline" size="sm">
                  <Crown className="mr-2 h-4 w-4 text-amber-500" /> PDF Premium
                </Button>
              </Link>
            )}
          </div>
        ...
        {accessDenied ? (
          <div className="rounded-3xl border bg-card p-12 text-center shadow-sm">
            ...
            <p className="mt-2 text-sm text-muted-foreground">Vous avez consulté 3 leçons aujourd'hui. Revenez demain ou passez Premium.</p>
            <div className="mt-6 flex flex-col items-center gap-3">
              <Link href="/dashboard/upgrade"><Button className="bg-hero-gradient text-white hover:opacity-90"><Crown className="mr-2 h-4 w-4" />Passer Premium</Button></Link>
              <Link href={backUrl}><Button variant="ghost">Retour aux leçons</Button></Link>
            </div>
          </div>
        ) : (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="rounded-3xl border bg-card shadow-sm">
            <div className="p-8 pb-4">
              <p className="text-xs font-bold uppercase text-blue-600 tracking-wider">{subject || "Cours"}</p>
              <h1 className="text-3xl font-bold mt-2">{title}</h1>
              {pdfUrl && isPremium && (
                <a href={pdfUrl} target="_blank" rel="noreferrer" className="mt-2 inline-flex items-center gap-1.5 text-xs text-blue-600 hover:underline">
                  <Download className="h-3.5 w-3.5" /> Fichier PDF original
                </a>
              )}
            </div>
            <div className="border-t p-8 pt-6">
              <article
                className="prose prose-blue max-w-none dark:prose-invert prose-headings:font-bold prose-h1:text-2xl prose-h2:text-xl prose-h3:text-lg prose-table:w-full prose-th:bg-blue-600 prose-th:text-white prose-th:px-3 prose-th:py-2 prose-td:px-3 prose-td:py-2 [&_svg]:w-full [&_svg]:h-auto [&_img]:w-full [&_img]:h-auto"
            ...
            <div className="border-t p-6">
              {completed ? (
                <div className="flex items-center justify-center gap-2 rounded-2xl bg-emerald-500/10 p-4">
                  ...
                  <p className="font-semibold text-emerald-700">Leçon validée ! Bravo 🎉</p>
                </div>
              ) : (
                <Button
